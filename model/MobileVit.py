...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def forward_features_save(self, x, indexes=None):
        ...
        #         y = blk.transformer[-1].norm1(x)
        #         B, N, C = y.shape
        #         qkv = blk.transformer[-1].attn.qkv(y).reshape(B, N, 3, blk.transformer[-1].attn.num_heads, C // blk.transformer[-1].attn.num_heads).permute(2, 0, 3, 1, 4)
        #         q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        #         att = (q @ k.transpose(-2, -1)) * blk.transformer[-1].attn.scale
...
@register_model
def deit_small_distilled_patch16_224(pretrained=False, img_size=(224,224), num_classes =1000, **kwargs):
    model = DistilledVisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=384, num_classes=num_classes, depth=1, num_heads=6, mlp_ratio=4, qkv_bias=True,
